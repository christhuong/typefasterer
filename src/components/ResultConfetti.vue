...
class Confetti {
  ...
  reset() {
    this.done = false;
    this.startPos.x = (1 - Math.round(Math.random())*2)*window.innerWidth/2;
    ...
    this.angle = 0;
    this.dx = window.innerWidth < window.innerHeight ? 3 + Math.random()*2 : 4 + Math.random()*4;
    this.dy = -8;
...
export default {
  ...
  data() {
    return {
      numberOfConfetti: 20,
      colors: ['#00afd7', '#9bf0e1', '#ff5f5f', '#40ff40', '#f573a0', '#ffa845'],
      bits: [],
      raf: null
    }
  },
