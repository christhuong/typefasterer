...
export default {
  ...
  methods: {
    ...
    optionsClicked() {
      this.selectingOptions = !this.app.running || this.app.paused ? true : false
    },
    getTime(event) {
      this.displayTime = false;
      this.selecting = false;
      this.setTimeAllotted(event.target.value >= 60 ? 60*60 : event.target.value*60);
    }
  },
  computed: {
    ...mapState({
    ...
    timeLabel() {
      return !this.app.running && this.time.left === 0 ? this.text.time : this.timeString
    }
  },
  mounted() {
