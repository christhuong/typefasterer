...
export default {
  ...
  methods: {
    ...
    typeWriter(no) {
      ...
      if (this.welcomeIndex[no] < this.welcomeText[no].length) {
        if (no === 0) this.animatedText0 += this.welcomeText[no].charAt(this.welcomeIndex[no]);
        if (no === 1) this.animatedText1 += this.welcomeText[no].charAt(this.welcomeIndex[no]);
  ...
  mounted() {
    if (this.app.mode === 'test' && this.text === '') this.setText(this.defaultTestText);
    this.guideParent = document.querySelector('#app-text-display h1');
